...
def run():
    ...
    if not os.path.exists("NI-SB_data.npy"):
        trail_data = np.zeros((21,5,4))
        for bd in ['b','Nb','d','Nd']:
            for i in [1,2,3,4,5]:
                print('num= ',i)
                sampling_time=[0]
                ...
                for k in range(20):
                    ...
                    if np.isclose(hv_max, hv, rtol=1e-14, atol=1e-06):
                        break
                if bd=='b':
                    trail_data[:,i-1,0] = np.array(hv_list)
                elif bd=='Nb':
                    trail_data[:,i-1,1] = np.array(hv_list)
                elif bd=='d':
                    trail_data[:,i-1,2] = np.array(hv_list)
                elif bd=='Nd':
                    trail_data[:,i-1,3] = np.array(hv_list)

        np.save('all_plot/raw_data/NI-SB_data.npy',trail_data)
